...
public class LuaScriptLoader {
	...
	static void RefreshAllLuaScripts()
	{
		...
		//var pathGeneratedScripts = new System.Uri(Path.Combine(Application.dataPath, "_Generated/_ScriptLua/"));

		var pathModules = new System.Uri(Path.Combine(Application.dataPath, "Plugins/Common/Libs/Unity3D.Lua/Modules/"));
		...
		var scripts = 
			...
			/*
			.Union(
				Directory.GetFiles(pathGeneratedScripts.AbsolutePath, "*.lua", SearchOption.AllDirectories)
				.Select(p => new KeyValuePair<string, string>(pathGeneratedScripts.MakeRelativeUri(new System.Uri(p)).ToString(), p))) 
			*/
			.Union(
	...
	static byte[] LoadLuaScriptRunTime(string scriptName, out string scriptPath)
	{
		...
		var s = Resources.Load<TextAsset>("_LuaRoot/" + scriptName);
		if (s != null)
		{
	...
	{
		if (Application.isEditor && !BuildEnv.runScriptInPackage)
		{
			// debugging in	editor
	...
	public static Type TypeLoader(string typename)
	{
		...
		if (type == null && typename.IndexOf(',') == -1)
		{
			...
			type = type != null ? type : Type.GetType(string.Format("{0}, PlayMaker", typename));
	...
	{
		return new string[] {
			"Plugins/Common/Libs/Unity3D.Lua",
		};
	}
